    ...
    <>
      <motion.nav
        ...
        className={` top-0 left-0 right-0 ${
          isAssignmentsPage.isAssignmentsPage ? "z-0 sticky " : "z-50 fixed"
        }  backdrop-blur-md bg-white/60 dark:bg-black/40 border-b border-white/20 dark:border-gray-700 shadow-sm`}
      >
        <div className="max-w-7xl mx-auto px-4 py-2 flex justify-between items-center">
          {/* Logo + rotation */}
          <Link
            to="/"
            className="relative flex items-center gap-2 text-xl font-semibold text-gray-800 dark:text-white transition-all duration-300 group"
          >
            {/* Subtle animated blur glow */}
            <div className="absolute -inset-1 opacity-0 group-hover:opacity-100 blur-md rounded-xl bg-gradient-to-r from-emerald-300/20 to-cyan-300/20 transition-all duration-500" />

            {/* Subtle floating emoji with depth */}
            <motion.span
              className="text-2xl"
              animate={{ y: [0, -2, 0] }}
              transition={{
            ...
            <span className="hidden md:inline-block relative z-10 bg-clip-text text-transparent bg-gradient-to-r from-gray-800 to-gray-600 dark:from-white dark:to-gray-300 group-hover:from-emerald-400 group-hover:to-cyan-400 transition-all duration-500">
              StudySync
            </span>
          </Link>
